...
export class Dashboard {
  ...
  private html(): string {
    ...
    return [
      ...
      '.shell{display:grid;grid-template-columns:minmax(0,1fr) 240px;gap:10px;height:calc(100vh - 28px);min-height:0}',
      '.main{min-width:0;min-height:0;display:grid;grid-template-rows:auto auto auto 1fr;gap:10px}',
      ...
      '.stat .v{font-size:18px;line-height:1;color:var(--vscode-foreground)}',
      ...
      '.grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;min-height:0}',
      ...
      '#grid-top .box .scroll{max-height:180px}',
      '#grid-bottom .box .scroll{max-height:190px}',
      ...
      '.callout{margin-top:10px;padding:10px 12px;border-radius:6px;border:1px solid color-mix(in srgb, var(--vscode-errorForeground) 35%, transparent);background:color-mix(in srgb, var(--vscode-errorForeground) 10%, transparent);font-size:12px;line-height:1.45}',
      '.callout strong{display:block;margin-bottom:4px;color:var(--vscode-foreground)}',
      ...
      '.hero-metric{padding:8px 10px;border-radius:6px;background:color-mix(in srgb, var(--vscode-badge-background) 45%, transparent);font-size:12px;min-width:110px}',
      '.hero-metric .label{display:block;font-size:10px;letter-spacing:.08em;text-transform:uppercase;color:var(--vscode-descriptionForeground);margin-bottom:4px}',
      ...
      '@media (max-width: 1280px){.stats{grid-template-columns:repeat(3,minmax(0,1fr))}.grid{grid-template-columns:1fr}.grid.two{grid-template-columns:1fr}}',
      '@media (max-width: 860px){body{overflow:auto}.shell{grid-template-columns:minmax(0,1fr);height:auto}.main{display:block}.side{order:-1;overflow:visible}.stats{grid-template-columns:repeat(2,minmax(0,1fr))}#grid-top .box .scroll,#grid-bottom .box .scroll{max-height:none}}',
      '</style></head><body>',
      '<div class="hdr">',
      ...
      'const blocker=(d.timeline||[]).slice().reverse().find((e)=>e.type==="needs_decision"||e.type==="state_changed");',
      'const timeline=(d.timeline||[]).slice(-6).reverse().map((e)=>`<div class="timeline-item"><span class="time">${esc(e.timestamp?e.timestamp.slice(11,16):"")}</span><span class="event">${esc(e.summary||e.type||"")}</span></div>`).join("")||`<div class="empty">No timeline events yet.</div>`;',
      ...
      'const totalAdded=(d.changedFiles||[]).reduce((n,f)=>n+(f.added||0),0);',
      'const totalRemoved=(d.changedFiles||[]).reduce((n,f)=>n+(f.removed||0),0);',
      'const ev=d.evidence||{};',
      ...
      'const evidenceCallout=!ev.summary_exists&&!ev.pytest_exists?`<div class="callout"><strong>Evidence bundle missing</strong>This worker never wrote summary or pytest artifacts, so Relay cannot close it out or open a reviewable PR.</div>`:"";',
      ...
      'const transcriptCallout=(d.transcriptPreview||[]).length?`<div class="callout"><strong>Latest worker output</strong>${esc(d.transcriptPreview[(d.transcriptPreview.length-1)]||"")}</div>`:"";',
      'const metrics=`<div class="hero-metrics"><div class="hero-metric"><span class="label">Changed files</span>${(d.changedFiles||[]).length}</div><div class="hero-metric"><span class="label">Diff lines</span><span class="add">+${totalAdded}</span> <span class="del">-${totalRemoved}</span></div><div class="hero-metric"><span class="label">Evidence</span>${ev.summary_exists||ev.pytest_exists?"partial":"missing"}</div></div>`;',
      'byId("grid-top").innerHTML=`<div class="panel box"><h3>Timeline</h3>${blocker?`<div class="callout"><strong>Current blocker</strong>${esc(blocker.summary||blocker.type||"")}</div>`:""}<div class="scroll">${timeline}</div><a class="link" href="#" data-session-cmd="viewTimeline">Open full timeline</a></div><div class="panel box"><h3>Changed Files</h3>${metrics}<div class="scroll">${files}</div>${d.diffAvailable?`<a class="link" href="#" data-session-cmd="viewDiff">View diff</a>`:""}</div><div class="panel box"><h3>Evidence</h3>${evidenceCallout}<div class="scroll">${evidence}</div><a class="link" href="#" data-session-cmd="viewEvidence">Inspect evidence</a></div>`;',
      'byId("grid-bottom").innerHTML=`<div class="panel box"><h3>Transcript (latest)</h3>${transcriptCallout}<div class="scroll">${transcript}</div><a class="link" href="#" data-session-cmd="peek">Open full transcript</a></div><div class="panel box"><h3>Operator Notes</h3><div class="scroll"><div class="callout"><strong>What to do next</strong>Use Refresh to resync state from disk, Peek to inspect the full transcript, and Attach if you want to step into the live terminal. For quota-blocked runs, the owner usually needs to restore credits or redirect the work.</div></div></div>`;',
