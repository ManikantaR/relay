...
function ctx(): { prefix: string; command: string; cwd: string } {
  ...
  return {
    prefix: cfg.get<string>('execPrefix', ''),
    command: cfg.get<string>('command', 'relay'),   // absolute path if not on the GUI PATH
    cwd,
  };
}
...
export function runRelay(args: string): Promise<string> {
  const { prefix, command, cwd } = ctx();
  const cmd = `${prefix} ${command} ${args}`.trim();
